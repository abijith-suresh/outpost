...
export class RepoShowError extends Schema.TaggedError<RepoShowError>()("RepoShowError", {
...
export function runRepoShow(
...
): Effect.Effect<CommandOutput, RepoShowError, FileSystem.FileSystem | Path.Path> {
  return Effect.gen(function* () {
    ...
    if (matches.length === 0) {
      return yield* Effect.fail(new RepoShowError({ message: `Unknown repo id: ${repoId}` }));
    }
